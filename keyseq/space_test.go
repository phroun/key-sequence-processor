...
func TestSpaceChordResolves(t *testing.T) {
	sp := NewProcessor(nil)
	sp.SetMappings(map[string]string{"^B space": "chord_target"})

	if r := sp.ProcessKey("^B"); r.Command != "" {
		t.Fatalf("^B should open a sequence, not fire a command; got %q", r.Command)
	}
	if r := sp.ProcessKey("space"); r.Command != "chord_target" {
		t.Fatalf(`"^B space" should resolve to chord_target; got %q`, r.Command)
	}
	...
	sp.ProcessKey("^B")
	if r := sp.ProcessKey(" "); r.Command == "chord_target" {
