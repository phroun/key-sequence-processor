...
// testDefaultHandler stands in for an application's unbound-key policy: a
// single typed character inserts itself, and a Glyph chord (the AltGr/Level3
// modifier) inserts the composed character it carries. The processor supplies
// no policy of its own, so these tests install one and then check it is
// consulted at the right moments — and, crucially, only after every binding
// has declined.
func testDefaultHandler(key string) string {
	if len([]rune(key)) == 1 {
		return "insert '" + key + "'"
	}
	if glyph, ok := strings.CutPrefix(key, "G-"); ok && len([]rune(glyph)) == 1 {
		return "insert '" + glyph + "'"
	}
	return ""
}

func newCaptureSP(mappings map[string]string, decline ...string) (*Processor, *captureHarness) {
...
// layer is that a hosted terminal's wildcard takes keys the base set has
...
// Escape belongs to the child, not that it does unless a lower level has an
// esc-chord.
